...
# Re-touched to use pyglet instead of pygame for image/texture loading.
...
class MTL(object):
    ...
    def bind(self, material):
        if material:
            ...
            if 'texture' in mtl:
                glBindTexture(mtl["texture"].target, mtl["texture"].id)
            else:
                glBindTexture(GL_TEXTURE_2D, 0)

                if glIsEnabled(GL_LIGHTING):
                    glMaterialfv(GL_FRONT_AND_BACK, GL_AMBIENT, vec(*mtl['Ka']))
                    glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, vec(*mtl['Kd']))
                    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, vec(*mtl['Ks']))
...
class OBJ(object):
    ...
    def parse(self, filename):
        ...
        self.texcoords = []
        self.mfaces = []
        self.vmin = Vector3(float_info.max, float_info.max, float_info.max)
        self.vmax = Vector3(float_info.min, float_info.min, float_info.min)
        material = None
        ...
        for line in lines:
            ...
            if not values: continue
                ...
                if len(values) < 2:
                    ...
                    print 'Non-implemented: Use white material'
                else:
                    material = values[1]
            elif values[0] == 'mtllib':
                pos = filename.find('/') + 1
                self.mtl = MTL(filename[0:pos] + values[1])  # TODO: multiple files?
            elif values[0] == 'p':
                ...
                print "Non-impemented OBJ element %s" % values[0]
